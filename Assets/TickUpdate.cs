...
public class TickUpdate : MonoBehaviour {
    ...
    public int   entityAgeWhenBorn    = -400;
    public int   entityAdultAge       = 0;
    public float entityYouthScale     = 0.75f;
	...
	public void updateStaticObjects() {
        ...
        for (int i=0; i < staticObjectsPerTick; i++) {
            ...
            if (Vector3.Distance(chunkPos, playerPos) > totalDistance) {
                ...
                // Update LOD
                if (staticObject.transform.childCount > 1) {
                    if (staticObject.transform.GetChild(1).name == "lod") {
                        playerPos.y = staticObject.transform.GetChild(1).transform.position.y;
                        staticObject.transform.GetChild(1).LookAt( playerPos );
                    }
	...
	public void updateEntities() {
        ...
        for (int i=0; i < entityObjectsPerTick; i++) {
            ...
            // Grow up to adult
            if (entityTag.Age > entityAdultAge) {
                
                if ((entityObject.transform.localScale.x < 1f) & 
                    (entityObject.transform.localScale.y < 1f) & 
                    (entityObject.transform.localScale.z < 1f)) {
                    
                    entityObject.transform.localScale = new Vector3(geneTag.adultSizeMul, geneTag.adultSizeMul, geneTag.adultSizeMul);
                }
            }
            
            
            // Entity expiration in old age
            if (allowEntityExpiration) {
                
                if (entityTag.Age > entityOldAge) {
                    entityTag.Health -= Random.Range(0, 2);
            ...
            if (doDebugMode) {
                if (actorTag.targetEntity == playerObject) {
	...
	public void unloadChunks() {
        ...
        if ((!chunkSerializer.isSaving) & (!chunkSerializer.isChunkSaved)) {
            ...
            float farRenderDistance  = RenderDistance * 100f * 2.5f;
	...
	public void entityAutoBreeding(GameObject entityList) {
        ...
        if (!allowCrossBreeding) {
            if (geneSrcM.GetComponent<EntityTag>().name != geneSrcP.GetComponent<EntityTag>().name) 
                return;
        }
	...
	public void joinGeneticPair() {
        ...
        // Must be of age
        if ((entityTagM.Age < entityAdultAge) | (entityTagP.Age < entityAdultAge)) 
            return;
        ...
        newEntity.transform.localScale = new Vector3(entityYouthScale, entityYouthScale, entityYouthScale);
        youthEntityTag.Age = entityAgeWhenBorn;
        ...
        if (Random.Range(0, 1) == 1) {offspring.limbAxisInvert = actorTagP.limbAxisInvert;} else {offspring.limbAxisInvert = actorTagM.limbAxisInvert;}
	...
	public void saveCurrentStructureToFile(string filename) {
        ...
        structBuild.structureHeight = structureHeight;
    ...
    string consoleRule(string[] paramaters) {
        ...
        if (ruleName == "structure") {
            if (status=="true")  {chunkGenerator.doSavePlacedObjectsToStructure = true; clearCurrentStructure(); return "Structure saving enabled";}
            if (status=="false") {chunkGenerator.doSavePlacedObjectsToStructure = false; clearCurrentStructure(); return "Structure saving disabled";}
        }
        ...
        if (ruleName == "oldage") {
            if (status=="true")  {entityOldAge = 1000;  return "Entity death enabled";}
            if (status=="false") {entityOldAge = -1;    return "Entity death disabled";}
        }
    ...
    string consoleStructure(string[] paramaters) {
        ...
        if (paramaters[1] == "place") {
            ...
            playerPos.y = Mathf.Round(playerObject.transform.position.y);
