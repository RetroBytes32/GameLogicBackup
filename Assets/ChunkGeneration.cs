...
public class Biome {
    ...
    public float  region_edgemelt_x;
...
public class TreeSpawn {
	...
	public float  stackHeightMin;
	public float  stackHeightMax;
...
public class ChunkGeneration : MonoBehaviour {
            ...
            for (int x=0; x <= 100; x++) {
                ...
                if (biomes.Length > 0) {
                    ...
                    for (int a=0; a < biomes.Length; a++) {
                        ...
                        float xCoordA = (((float)x * biomes[a].region_mul_x) + (chunk_x * biomes[a].region_mul_x) + x_offset + biomes[a].region_x);
                        float zCoordA = (((float)z * biomes[a].region_mul_z) + (chunk_z * biomes[a].region_mul_z) + z_offset + biomes[a].region_z);
                        
                        float sampleA = ((Mathf.PerlinNoise(xCoordA, zCoordA) * 2) - 1) * biomes[a].region_mul_y;
                        
                        // Add some roughness to the biome edges
                    ...
                    //color_total = Color.Lerp(biomes[underBiomeIndex].base_color, biomes[biomeIndex].base_color, biomeNoiseSample * 10f);
                    ...
                    if (noise_sub_total > biomes[biomeIndex].Mountain_rise) {
                        ...
                        //color_total = Color.Lerp(biomes[underBiomeIndex].base_color, biomes[biomeIndex].Mountain_color, (noise_sub_total - biomes[biomeIndex].Mountain_rise) * biomes[biomeIndex].Mountain_color_mult);
    ...
    public void generateVillage(GameObject chunk, float xCoordVillage, float zCoordVillage) {
        ...
        for (int c=0; c < villageNumberOfBuilds; c++) {
            ...
            if (biomes[chunkTag.biome].entityVariants.Length > 0) {
                ...
                bool currentSexuality = false;
                
                for (int b=0; b < numberToSpawn; b++) {
                    
                    entityVariant = Random.Range(0, biomes[chunkTag.biome].entityVariants.Length);
                    ...
                    geneticTag.masculine = currentSexuality;
                    currentSexuality = !currentSexuality;
	...
	public bool generateStaticObject(float x, float y, float z, GameObject chunk, ChunkTag chunkTag, int biomeIndex) {
        ...
        if ((y < biomes[biomeIndex].staticList[static_type].heightRangeMin) | (y > biomes[biomeIndex].staticList[static_type].heightRangeMax))
            return false;
	...
	public bool generateTreeObject(float x, float y, float z, GameObject chunk, ChunkTag chunkTag, int biomeIndex) {
        ...
        if ((y < biomes[biomeIndex].treeSpawn[tree_type].heightRangeMin) | (y > biomes[biomeIndex].treeSpawn[tree_type].heightRangeMax))
            return false;
        ...
        float maxStackingHeight = biomes[biomeIndex].treeSpawn[tree_type].stackHeightMax;
	...
	public void generateDymanicEntity(float x, float y, float z, GameObject chunk, ChunkTag chunkTag, int biomeIndex) {
        ...
        if ((y < biomes[biomeIndex].entities[dynamic_type].heightRangeMin) | (y > biomes[biomeIndex].entities[dynamic_type].heightRangeMax))
            return;
