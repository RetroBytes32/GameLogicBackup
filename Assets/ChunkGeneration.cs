...
public class ChunkGeneration : MonoBehaviour {
	...
	public GameObject generateChunk(float chunk_x, float chunk_z, float chunk_sz, bool addStaticObjects, bool forceGenerate, bool dummyChunk) {
	    ...
	    for (int x=0; x <= 100; x++) {
	      ...
	      if (biomes.Length > 0) {
	        ...
	        for (int a=0; a < biomes.Length; a++) {
              ...
              float newBiomeSample = ((Mathf.PerlinNoise(xCoord, zCoord) * 2) - 1) * biomes[a].region_mul_y;

	          if (newBiomeSample > biomeNoiseSample) {

                biomeNoiseSample = newBiomeSample;
                biomeIndex = a;
            ...
            if (biomes[biomeIndex].Mountain_cap > 0) {
	      ...
	      //
	      // Add static and dynamic world objects
	      //
	      ...
	      if (noise_total > 0f) {
	        ...
	        if ((addStaticObjects) & (noise_total < MountainCapHeight)) {
	          ...
	          if (addWorldEntities) generateDymanicEntity(x, noise_total, z, currentChunk, chunkTag, biomeIndex);
