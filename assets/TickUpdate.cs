...
public class TickUpdate : MonoBehaviour {
	...
	public void updateEntities() {
        ...
        for (int i=0; i < entityObjectsPerTick; i++) {
            ...
            // Grow up to adult
            if ((entityObject.transform.localScale.y < geneTag.adultSizeMul) & (entityTag.Age < entityAdultAge)) {
                
                float growthStep = geneTag.adultSizeMul * 0.0008f;
	...
	public bool loadChunks() {
        ...
        if (canLoadChunk == false) {
            ...
            if ((ChunkCounterX == 0) & (ChunkCounterZ == 0)) {
                ...
                if (currentChunkRing >= (RenderDistance * 0.8f)) 
                    currentChunkRing = 2;
            }
